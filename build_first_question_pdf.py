...
def read_rows(relative):
    with (RESULT / relative).open(encoding='utf-8-sig', newline='') as stream:
        return list(csv.DictReader(stream))


def build():
    intervals = read_rows('汇总与说明/四组核心指标重采样区间.csv')
    stability = read_rows('汇总与说明/左右差分前后时段稳定性.csv')
    datasets = [('VisualCogA_Task-1', 'A · 项目一', '90 / 100'),
                ('VisualCogA_Task-2', 'A · 项目二', '95 / 100'),
                ('VisualCogB_Task-1', 'B · 项目一', '94 / 100'),
                ('VisualCogB_Task-2', 'B · 项目二', '92 / 100')]
    def interval(dataset, metric):
        return next(row for row in intervals if row['dataset'] == dataset and
                    row['stage'] == 'V7' and row['metric'] == metric)
    def contrast(dataset):
        return next(row for row in stability if row['dataset'] == dataset and
                    row['stage'] == 'V7' and row['channel'] == '三通道合并')
    ...
    add(p('核心结果', H1))
    rows = [['数据组', '可用试次', '代理 MAE 降幅', 'SNR 代理增量', '左右差分幅度比']]
    for ds, name, n in datasets:
        mae = interval(ds, 'proxy_MAE_reduction_pct')
        snr = interval(ds, 'SNR_proxy_gain_dB')
        retention = interval(ds, 'left_right_retention_ratio')
        rows.append([name, n, f"{float(mae['point']):.1f}%",
                     f"{float(snr['point']):+.3f} dB", f"{float(retention['point']):.3f}"])
    add(table(rows, [86, 78, 104, 106, 114]))
    add(Spacer(1, 8 * mm))
    add(p('解读原则', H2))
    story += bullets(
        'V7 四组代理 MAE 降幅的同试次重采样区间均高于零；SNR 增量的四组点估计均为正，但区间都跨零。',
        '左右差分幅度比接近 1 仅表明与处理前更相近；处理前可能包含方向相关眼动。',
        'A 项目一与 B 项目二的左右差分前后半段相关为负，特征稳定性仍不足。1 倍、2 倍半合成扰动较 V6 略好，零注入与轻扰动仍有代价。',
    )
    add(p('结论定位', H2))
    add(p('这是同一批四组数据上的开发集折外实验，不是独立受试者泛化结果。试次区间固定已选模型和训练代理参考，仅描述本组抽样波动。数据只有三个额区通道，缺少眼电和独立无伪影脑电真值，不能证明神经源无损恢复。'))
    add(PageBreak())
    ...
    add(p('方向差分的前后时段稳定性', H2))
    temporal_rows = [['数据组', '前半左 / 右', '后半左 / 右', '三通道差分相关']]
    for ds, name, _ in datasets:
        item = contrast(ds)
        temporal_rows.append([name,
                              f"{item['first_left_n']} / {item['first_right_n']}",
                              f"{item['second_left_n']} / {item['second_right_n']}",
                              f"{float(item['contrast_correlation']):+.3f}"])
    add(table(temporal_rows, [110, 119, 119, 140]))
    add(Spacer(1, 4 * mm))
    add(p('按原始事件编号对半，每半分别计算 250–500 ms 左减右 ERP，再对三通道拼接波形求相关。A 项目一和 B 项目二为负值；差分幅度接近处理前也不能据此认定视觉特征得到可靠保留。', SMALL))
    add(PageBreak())

    add(p('2  校正幅度与特征保留的联合评价', H1))
    add(figure(RESULT / '汇总与说明/去噪与特征保留联合评价.png', max_height=190 * mm))
    add(Spacer(1, 3 * mm))
    add(p('图 1｜方块为 V6、圆点为 V7；横线为 95% 分方向试次重采样区间。灰色虚线表示无变化或差分比为 1。三指标含义不同，不合成单一分数。区间固定模型、剔除集和训练代理参考。', CAP))
    add(PageBreak())
    ...
    story += bullets(
        ...
        '四组结果经过开发过程观察；五折只能保证单次外折测试试次不参与该折模板和策略估计。重采样区间固定模型与代理参考，不能消除开发集调参偏倚，也不能推断人群或临床效果。',
        'A 项目一和 B 项目二的前后时段左右差分相关为负；四组 SNR 代理增量的 95% 区间均跨零。逐通道时段稳定性和核心指标区间在汇总 CSV 中。',
