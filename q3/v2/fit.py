...
class Bank:
    def __init__(self,ds,params,context=False):
        ...
        for i,e in enumerate(ds.events):
            block=e['block'];h=design_bank(params,e,context=context).astype(float)
    ...
    def matrix(self,indices,candidate_index):
        ...
        for i in indices:
            h=design_bank([c],self.ds.events[i],context=self.context)[0].astype(float)
