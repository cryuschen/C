...
import numpy as np
from q3.data import FS,TIME,build_trials,Trials,load_raw
from q3.v2.model import Candidate,states,design_bank,predict,penalties
...
class RevisedModelTests(unittest.TestCase):
    ...
    def test_past_context_is_strictly_pre_cue(self):
        ...
        with patch('q3.v2.data.load_raw',return_value=(changed,labels,path)):
            attach_past(ds)
        np.testing.assert_array_equal(expected,ds.events[0]['past_slopes'])
        self.assertEqual(first['past_last_sample'],first['cue_sample']-1)
