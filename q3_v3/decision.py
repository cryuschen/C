...
SCENARIOS=(
    ...
    ('low_evidence','低证据增益',REFERENCE,.15,.4,2.),
...
def simulate(candidate=REFERENCE,gamma=1.5,sigma=.6,deadline=2.,n=6000,
             seed=202609263,dt=1/FS,boundary=1.,event=EVENT):
    ...
    for j in range(len(time)):
        ...
        active=choice==0
        z[active]+=correct_side[active]*drift[j]*dt+sigma*np.sqrt(dt)*noise[active]
    ...
    summary=dict(n=n,tau_m=candidate.tau_m,tau_p=candidate.tau_p,
