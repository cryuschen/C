"""Portable V3 pipeline; writes only q3_result_v3 by default."""
from pathlib import Path
import sys,os,json,argparse,shutil,time
if __package__ in (None,''):
    sys.path.insert(0,str(Path(__file__).resolve().parents[1]))
ROOT=Path(__file__).resolve().parents[1]
os.environ.setdefault('MPLCONFIGDIR',str(ROOT/'q3_result_v3/.mplcache'))
import numpy as np
import pandas as pd
from threadpoolctl import threadpool_limits
from q3_v3.common import KEYS,TIME,FS,STAGES,save_json,save_csv,sha,stage_masks,load_raw
from q3_v3.events import build_trials,window_policy,product_limit
from q3_v3.model import Candidate,predict,states
from q3_v3.decision import SCENARIOS,simulate,EVENT,REFERENCE
...
def snapshot(out):
    inherited=out/'inherited_v2';inherited.mkdir(exist_ok=True)
    for src in sorted((ROOT/'q3_result_v2').iterdir()):
        if src.is_file():shutil.copy2(src,inherited/src.name)
    if (ROOT/'q3_result_v2/figures').exists():
        shutil.copytree(ROOT/'q3_result_v2/figures',inherited/'figures',dirs_exist_ok=True)
    paths=[*sorted((ROOT/'q3_v3').rglob('*.py')),ROOT/'q3_v3/README.md',ROOT/'q3_v3/requirements.txt',
           *sorted((ROOT/'Q2/q2model').glob('*.py')),*sorted((ROOT/'data').glob('*.mat')),
           ROOT/'服务于脑机接口与精神性疾病诊断的脑电图计算模型.docx']
    save_json(out/'manifest.json',dict(status='running',version='V3',
        source_hashes={str(p.relative_to(ROOT)):sha(p) for p in paths},
        inherited_hashes={str(p.relative_to(inherited)):sha(p) for p in inherited.rglob('*') if p.is_file()},
...
def past_slopes(key,ds):
    from q3_v3.common import causal_filter
    from q3_v3.events import audit_events
...
def eeg(out):
    ...
    for key in KEYS:
        ...
        for horizon in (1.5,2.,2.5):
            ...
            from q3_v3.common import Trials
...
def decisions(out):
    ...
    selected=[]
    for key in KEYS:
        fits=json.loads((out/f'inherited_v2/{key}_stimulus_choices.json').read_text(encoding='utf8'))
        for f in fits:
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--stage',choices=['all','render'],default='all')
    p.add_argument('--output',type=Path,default=ROOT/'q3_result_v3');args=p.parse_args();out=args.output.resolve()
    if out in [(ROOT/n).resolve() for n in ('q3_result','q3_result_v2','q2_result','eeg_v7_results')]:
        p.error('禁止覆盖已有结果目录')
    out.mkdir(parents=True,exist_ok=True)
    with threadpool_limits(limits=1):
        if args.stage=='all':
            snapshot(out);eeg(out);decisions(out)
        from q3_v3.report import render
        render(out)
        m=json.loads((out/'manifest.json').read_text(encoding='utf8'));m['status']='computed';save_json(out/'manifest.json',m)
        from q3_v3.audit import audit
        audit(out)
    log('V3 计算、报告与数值审计完成')
