...
import json
import numpy as np
from q3_v3.common import load_raw,FS,TIME,causal_filter
from q3_v3.events import audit_events,window_policy,build_trials,product_limit
from q3_v3.model import Candidate,states,predict
from q3_v3.decision import simulate,aligned_evidence,REFERENCE
...
class UnifiedModelTests(unittest.TestCase):
    ...
    def test_V2_fitted_waveform_reproduced(self):
        ds,_=build_trials('A2');choices=json.loads((ROOT/'q3_result_v2/A2_stimulus_choices.json').read_text(encoding='utf8'))
        f=next(c for c in choices if c['model']=='N2' and c['held_block']==0)
        i=int(np.flatnonzero(ds.blocks==0)[0]);old=np.load(ROOT/'q3_result_v2/A2_stimulus_waves.npz')
