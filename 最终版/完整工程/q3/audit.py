...
import numpy as np
import pandas as pd
from q3.common import ROOT,KEYS,save_json
from q3.reference import verify_reference
...
def audit(out):
    ...
    assert manifest['reference_hashes']==verify_reference(out/'reference')
    for path,h in manifest['reference_hashes'].items():
        assert digest(out/'reference'/path)==h,path
    ...
    for key in KEYS:
        for setting in ('stimulus','past'):
            ...
            with np.load(out/f'reference/{key}_{setting}_waves.npz') as archive:
            ...
            fits=json.loads((out/f'reference/{key}_{setting}_choices.json').read_text(encoding='utf8'))
            for f in fits:
    ...
    result=dict(status='passed',n_real_events=len(ev),n_main_EEG_trials=int(ev.retained.sum()),
        ...
        n_new_scenarios=len(ds),source_and_reference_hashes_match=True,
        frozen_reference_files_unchanged=True,neural_parameters_refitted=False,
    ...
    manifest['output_hashes']={str(p.relative_to(out)):digest(p) for p in sorted(out.rglob('*'))
        ...
        and p.name!='运行日志.txt' and '.mplcache' not in str(p) and 'reference' not in p.parts}
