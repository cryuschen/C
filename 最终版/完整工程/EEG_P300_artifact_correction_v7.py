...
os.environ.setdefault('MPLCONFIGDIR',str(Path(tempfile.gettempdir())/'eeg-v7-mpl'))
import argparse
import hashlib
import json
import platform
...
def write_report(out,datasets,summary,spatial,benchmark,fitrows,group_intervals,stability):
    ...
    if (out/'独立验证_报告.md').exists():
        lines += ['', '## 新增独立验证', '', '另见[独立验证报告](独立验证_报告.md)：275次独立时间块队列、未参与选型的污染家族、简单收缩及训练门控对照。此分支与本报告371次历史队列分开；不得直接比较不同队列的指标。']
...
def main():
    ...
    parser.add_argument('--independent',action='store_true',help='独立时间块、未知污染族、简单收缩与保守门控对照')
    args=parser.parse_args()
    out=args.output.resolve()
    if args.independent:
        from Q1.independent_validation import run
        run(out)
        return
