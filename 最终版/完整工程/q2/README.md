...
- 默认 1999 次块内置换、199 次循环移位、1000 次块重采样。maxT 只校正本次七组特征，不能覆盖历史探索。

```bash
# 快速检查，会覆盖本轮同名文件；正式论文请随后完整运行
.venv/bin/python -B q2/main.py --quick
# 从已保存数值重绘、生成论文并核验
.venv/bin/python -B q2/main.py --redraw
# 只计算，随后可 --redraw
.venv/bin/python -B q2/main.py --no-plots
```
...
新增固定六维/三维投影，七组主特征共同重算1999次块内置换和199次循环移位。原机制特征分数未变，校正p值随比较范围变化。详见[优化报告](../q2_result/Q2_优化报告.md)。方向收缩未替换原主模型。
