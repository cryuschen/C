...
def build_unsupervised_reference_model(epochs, cues, irrecoverable, artifact_scores, ref_ratio=0.50):
    """
    无监督：全局构建低伪影 ERP 模板及动态残差尺度
    """
    templates = {}
    scales_v = {}
    scales_h = {}
    ...
    valid_idx = np.where(~irrecoverable)[0]
    if len(valid_idx) > 0:
        sorted_idx = valid_idx[np.argsort(artifact_scores[valid_idx])]
        k = max(8, int(np.ceil(ref_ratio * len(sorted_idx))))
        chosen = sorted_idx[:k]
        ...
        tpl = np.zeros((3, epochs.shape[2]))
        scale_v = np.ones(epochs.shape[2])
        scale_h = np.ones(epochs.shape[2])

    for cond in (-1, 1):
        templates[cond] = tpl.copy()
        scales_v[cond] = scale_v.copy()
        scales_h[cond] = scale_h.copy()
...
def select_candidate(x, cues, seed):
    ...
    model = build_unsupervised_reference_model(x[train],cues[train],np.zeros(len(train),bool),train_scores)
...
def training_mean_delta(x_train,cues_train,model,candidate):
    ...
    global_delta = delta.mean(axis=0)
    return {-1: global_delta, 1: global_delta}
...
def main():
    ...
    for subject in 'AB':
        for task in (1,2):
            ...
            eval_model = build_clean_reference_model(x, cues, np.zeros(len(x), bool), scores)
            refs={c: eval_model[0][c] for c in (-1,1)}
