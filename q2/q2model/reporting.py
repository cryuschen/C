...
FEATURE_NAMES={'amplitude':'均值振幅','peak_latency':'峰幅与潜伏期','spatial':'加入侧化',
               'mechanism':'加入机制','covariance':'协方差基线','past_only':'刺激前',
...
def render_all(out):
    ...
    for j,key in enumerate((*NAMES,'pooled')):
        sub=metric[(metric.dataset==key)&metric.features.isin(['amplitude','spatial','mechanism','past_only','post_given_past'])]
...
def write_paper(out):
    ...
    conclusions=[
        ...
        ('稳定特征区分能力',decoder_support,'图7及表4',f'机制特征 BA={row.BA:.3f}，AUC={row.AUC:.3f}，五组合校正 p={row.block_permutation_maxT_p:.4f}。')]
    ...
    lead=(f'本文构建图像驱动的三级兴奋—抑制群体模型，将三角形边缘的空间组合、LGN中继、皮层动态和额区头皮观测连接起来。'
          ...
          f'AUC为{row.AUC:.3f}，块内置换五特征组合maxT校正p={row.block_permutation_maxT_p:.4f}。'
...
'''
    sections=[('# 第二问 图像驱动的脑电形成模型与视觉响应表示',lead),
      ...
      ('## 特征表示及有效性',
       ...
       '它不需要未知试次的真实方向。模型时间窗特征不等于已确认的P300来源。\n\n'+
      ...
      ('## 统计检验与迁移',
       ...
       f'五项特征组合的BA采用maxT校正，并补充{manifest["circular_shifts"]}次记录内非零循环移位。'
       '循环移位后若某留出块只剩一类，按预先规定的可估计条件重新抽取，故该检验为条件随机化敏感性分析。'
       '判别区间对已经生成的折外预测进行时间块重采样，没有在每次重采样中重新训练；它不包含完整的模型训练不确定性。'
       'maxT覆盖本次五个特征组合，不覆盖此前反复开发过的全部模型，也不自动校正跨数据组选择。'
