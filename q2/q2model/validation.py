...
FEATURE_SETS=('amplitude','peak_latency','spatial','mechanism','covariance','past_only','previous_cue','post_given_past')
PRIMARY=FEATURE_SETS[:5]
...
def feature_fold(data,bank,y,train,test):
    ...
    mech=np.c_[spatial,mechanism_features(ds.x,pred)]
...
def bootstrap_metrics(prepared,outputs,n_boot,seed):
    ...
    for key in (*prepared,'pooled'):
        ...
        for name,vals in [('mechanism_minus_spatial',gains),('spatial_minus_peak',space),('residual_minus_past',past)]:
...
def transfer(train_data,test_data,bank):
    ...
    spatial,_=spatial_features(base,x,tr); mech=np.c_[spatial,mechanism_features(x,pred)]
