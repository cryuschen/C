#!/usr/bin/env python3
"""第一问 V6 可复核实验：固定试次上比较 V3、V4、保守校正比例的 V6。

运行：python EEG_P300_artifact_correction_v6.py --output eeg_v6_results
V6 属四组数据上的开发实验；任何试次区间都不是跨受试者泛化验证。
"""
import os
os.environ.setdefault('MPLCONFIGDIR','/tmp/eeg-v6-mpl')
import argparse
import hashlib
import json
import platform
...
STAGES=('预处理','V3','V4','V6')
COLORS={'原始':'#a8adb4','预处理':'#596e83','V3':'#c87927','V4':'#6b61a8','V6':'#007f89','代理参考':'#30343a'}
LINES={'原始':':','预处理':'--','V3':'-.','V4':':','V6':'-','代理参考':':'}
        ...
        for ch,channel in enumerate(CHANNELS):
            ...
            for metric in ('proxy_MAE','positive_mean'):
                ...
                for baseline in ('预处理','V3','V4'):
                    arr=boot['V6'][metric]-boot[baseline][metric]
                    lo,hi=np.quantile(arr,[.025,.975])
                    rows.append(dict(cue=condition,channel=channel,metric=metric,stage='V6减'+baseline,
                                     low=lo,high=hi,n_trials=len(ids),n_boot=n_boot))
    return rows
...
def draw_lines(ax,signals,mask=None,legend=True):
    ...
    for label,y in signals.items():
        ax.plot(TIMES[mask],np.asarray(y)[mask],label=label,color=COLORS[label],
                ls=LINES[label],lw=1.5 if label=='V6' else 1.)
...
def plot_condition(name,methods,cues,refs,out,zoom=False):
    ...
    subtitle='各图同一有效试次，同行左右共享纵轴；代理参考只作描述' if zoom else '四阶段同一试次；阴影是V6逐点95%试次重采样区间'
    ...
    for col,c in enumerate((-1,1)):
        ...
        for ch,channel in enumerate(CHANNELS):
            ...
            if not zoom:
                lower,upper=core.compute_bootstrap_ci(methods['V6'][cues==c,ch],n_boot=400)
                ax.fill_between(TIMES,lower,upper,color=COLORS['V6'],alpha=.14,label='V6逐点95%试次区间')
...
def plot_spatial(name,methods,cues,out):
    fig,axes=make_axes(name+' · 左右与额区差分','左列为左减右；右列为F3减F4及左右交互；各面板内四阶段同尺度；差分可能包含残余眼动')
    scale={k:[] for k in range(3)}
    for ch,channel in enumerate(CHANNELS):
...
def plot_fits(name,methods,cues,out):
    fig,axes=make_axes(name+' · V6 三通道分方向响应曲线拟合',
                       '固定50 ms结点三次样条；灰色为残差；窗内局部正峰只是描述量，不确认生理P300')
    records=[]
    for col,c in enumerate((-1,1)):
        n=int((cues==c).sum())
        for ch,channel in enumerate(CHANNELS):
            erp=methods['V6'][cues==c,ch].mean(0)
            fitted,fit=spline_fit(erp)
            ax=axes[ch,col]
            ax.plot(TIMES,erp,color=COLORS['V6'],label='V6 ERP')
            mask=(TIMES>=50)&(TIMES<=750)
...
def stage_summary_markdown(data,dataset):
    ...
    for metric in ('MAE','corr','SNR_proxy_dB','baseline_RMS','P300_positive_mean_error','P300_AUC_error_unit_ms'):
        ...
        if len(rows)==4:
            result.append('| '+metric+' | '+' | '.join(f'{rows.loc[s,"mean"]:.3f}' for s in STAGES)+' | '+str(int(rows.loc['V6','paired_rows']))+' |')
    return result
...
def write_report(out,datasets,summary,spatial,benchmark,fitrows):
    ...
    for ds,d in datasets.items():
        ...
        vals=[g[g.stage==s].retention_ratio.mean() for s in ('V3','V4','V6')]
        corr=g[g.stage=='V6'].waveform_correlation.mean()
    ...
    for ds,d in datasets.items():
        g=spatial[(spatial.dataset==ds)&(spatial.quantity=='left_minus_right_ERP')&(spatial.stage=='V6')]
        for ch in CHANNELS:
    ...
    for ds,d in datasets.items():
        ...
        m=d['metrics'];v=m[m.stage=='V6']
        paired=summary[(summary.dataset==ds)&(summary.metric=='amp_error')&(summary.stage=='V6')]
    ...
    lines += ['', '区间文件对每组每方向每通道进行400次试次重采样，固定处理后的波形与参考，提供95%逐指标区间和V6−基线配对差的区间。它仅量化本组试次有限样本波动，不能推断人群疗效或诊断能力。',
              '', '## 半合成闭环验证','',
    ...
    lines += ['', 'A项目一在0.5倍轻扰动时，V6恢复RMSE略高于未校正；这说明保守比例仍不能保证所有伪影强度都获益。0倍注入的非零恢复误差是算法对背景本身的改动，不能解释为恢复收益。不能将这些半合成结果推广为真实无噪声脑电恢复精度；没有专门的眼电或独立真值。各试次重复注入也不能当作独立受试者。',
              '', '## 图表检查与解释','',
...
def plot_cross_task(out,datasets):
    ...
    for subject in 'AB':
        fig,axes=make_axes('受试者'+subject+' · V6按方向跨项目ERP',
        ...
        for col,c in enumerate((-1,1)):
            for ch,channel in enumerate(CHANNELS):
                ...
                for task,color,style in ((1,'#007f89','-'),(2,'#c87927','--')):
                    ...
                    y=d['stages']['V6'][d['cues']==c,ch].mean(0)
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'eeg_v6_results')
    parser.add_argument('--no-plots',action='store_true',help='生成数值副本供重复性核验')
    args=parser.parse_args()
    out=args.output.resolve()
    if out==ROOT or any(out.is_relative_to(p) for p in (ROOT/'data',ROOT/'eeg_v2_results',ROOT/'eeg_v3_results',ROOT/'eeg_v4_results',ROOT/'eeg_v5_results')):
    ...
    for subject in 'AB':
        for task in (1,2):
            ...
            n=len(x);v3=np.empty_like(x);v4=np.empty_like(x);v6=np.empty_like(x);refs_trial=np.empty_like(x)
            ...
            for fold,(train,test) in enumerate(cv.split(x,cues),1):
                ...
                v6[test]=conservative_blend(selected,x[test],alpha)
                folds[test]=fold
                strategies.append(dict(dataset=ds,fold=fold,train_n=len(train),test_n=len(test),
                                       ...
                                       V6_base=method,correction_fraction=alpha,reference_count=len(reference_ids)))
                # 同试次、同注入、同训练折；V6先处理污染后的试次，再采用固定校正比例。
                for seed in (2027,2039,2053):
                    for level in (0.,.5,1.,2.):
                        ...
                        s5=conservative_blend(s3 if method=='V3' else s4,contaminated,alpha)
                        for stage,recovered in zip(STAGES,(contaminated,s3,s4,s5)):
                            ...
                            synthetic.append(dict(dataset=ds,fold=fold,seed=seed,level=level,stage=stage,
                                                  n_trials=len(test),V6_base=method,correction_fraction=alpha,**result))
            assert (folds>0).all()
            ...
            stages=dict(zip(STAGES,(x,v3,v4,v6)))
            ...
            np.savez_compressed(dest/'可复核波形.npz',raw=raw,before=x,v3=v3,v4=v4,v6=v6,cues=cues,
            ...
            if not args.no_plots:
                ...
                plot_examples(name,raw,stages,cues,scores,trial_ids,dest,.50,'中位伪影试次_全幅对照.png')
                plot_examples(name,raw,stages,cues,scores,trial_ids,dest,.75,'较高伪影试次_全幅对照.png')
                ...
                for c in (-1,1):
                    for ch,channel in enumerate(CHANNELS):
                        _,fit=spline_fit(stages['V6'][cues==c,ch].mean(axis=0))
            ...
            print(f'{name}：{len(audit)}事件，{n}可用；V6各折策略 '+', '.join(f'{r["V6_base"]}+{r["correction_fraction"]}' for r in strategies[-5:]),flush=True)
    save_csv(summary,summary_dir/'四阶段共同配对指标汇总.csv')
    save_csv(spatial,summary_dir/'左右刺激与额区空间差异指标.csv')
    save_csv(strategies,summary_dir/'逐折V6策略与参考数量.csv')
