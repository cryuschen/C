...
提交时先阅读 [第一问终稿实验报告](eeg_v7_results/第一问终稿实验报告.md)，再查看 `eeg_v7_results/` 中四组图表、逐方向逐通道指标、半合成验证和运行清单。完整的 44 张结果图（含三栏伪影分解图）由 `EEG_P300_artifact_correction_v7.py` 独立生成，不导入仓库中的 V3、V4 模块或单独的分解绘图脚本；运行时仍需安装 `requirements-v4.txt` 所列第三方库，并将原始四份 `.mat` 数据放在同目录的 `data/` 中。
